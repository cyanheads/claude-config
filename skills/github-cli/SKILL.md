...
# Key slash commands
/model          # Switch the active model (Claude Sonnet or GPT)
...
```bash
# Full context before reviewing — body, review summaries, and the comment thread
gh pr view 719 --comments

# The diff is the substance — body and comments rarely tell the whole story
gh pr diff 719
gh pr diff 719 --name-only      # changed files only, for scope

# Inline line-level review comments are NOT in --comments — fetch via the API
gh api 'repos/{owner}/{repo}/pulls/719/comments' \
  --jq '.[] | "\(.path):\(.line) — \(.user.login): \(.body)"'

# Cross-references — issues/PRs that mention this one (shared issue timeline endpoint)
