...
# `changelog/<major.minor>.x/<version>.md` (e.g. `changelog/0.2.x/0.2.1.md`)
...
# Optional free-form notes for agents upgrading a downstream consumer of this
# project. Not rendered in CHANGELOG — consumed only by an agent running an
# adoption/maintenance pass. Use for instructions that don't fit the
...
<!--
  ...
  inside its own UI, not in npm reads or local editors.
      ...
      [#38](https://github.com/<owner>/<repo>/issues/38)   ← issue
      [#42](https://github.com/<owner>/<repo>/pull/42)     ← PR
  ...
  DEPENDENCIES — routine bumps go in a `## Dependencies` section, grouped
  Runtime / Dev, one row per package with a version arrow:
  ...
  changelog entry — a condensed, scannable version, not a copy. Format:
    ...
    Added:
                                                          ← blank line
    - `manifest.json` scaffolded for bundle support
    - Install badges (Claude Desktop, Cursor, VS Code)
                                                          ← blank line
    <N> tests pass; `bun run check` clean.                ← footer
